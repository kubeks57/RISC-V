...
% Plot Average Time
subplot(3, 1, 1);
loglog(sizes1, avg_times1, '-o', sizes2, avg_times2, '-x');
grid on;
title('FFTW Processors Performance');
xlabel('Data Size');
ylabel('Avg. Time (s)');
legend('PC', 'RISC', 'Location', 'northwest');

% Plot Memory Usage
subplot(3, 1, 2);
loglog(sizes1, memory_usages1, '-o', sizes2, memory_usages2, '-x');
grid on;
xlabel('Data Size');
...
subplot(3, 1, 3);
loglog(sizes1, energy_consumptions_wh1, '-o', sizes2, energy_consumptions_wh2, '-x');
grid on;
xlabel('Data Size');
